...
public class MovieGraphApp {
	...
	private static void justvertex(String str, MovieGraph graph) throws ExceptionofInput {
		...
		if(info[1].equals("Actor")) {
			Actor r = (Actor) new ActorVertexFactory().createVertex(info[0],info);
		graph.addVertex(r);
		}
		else if(info[1].equals("Director")) {
			Director r = (Director) new DirectorVertexFactory().createVertex(info[0],info);
			graph.addVertex(r);
		}
		else if(info[1].equals("Movie")) {
			Movie r = (Movie) new MovieVertexFactory().createVertex(info[0],info);
			graph.addVertex(r);
		}else {
	...
	private static void justedge(String str,MovieGraph graph) throws ExceptionofInput, ExceptionofUnproperEdge {
		System.out.println("your input string is:\t"+str);
		...
		if(info[1].equals("MovieDirectorRelation")) {
			MovieDirectorRelation r = (MovieDirectorRelation) new MovieDirectorRelationFactory().createEdge(info[0], vers, weight);
			graph.addEdge(r);
		}
		else if(info[1].equals("MovieActorRelation")) {
			MovieActorRelation r = (MovieActorRelation) new MovieActorRelationFactory().createEdge(info[0], vers, weight);
			graph.addEdge(r);
		}else{
			
		}
	}
	private static void justremovev(String str,MovieGraph graph) {
		System.out.println("your input string is:\t"+str);
	...
	private static void justremovee(String str,MovieGraph graph) throws ExceptionofInput, ExceptionofUnproperEdge {
		System.out.println("your input string is:\t"+str);
	...
	private static void justhyper(String str,MovieGraph graph) {
		System.out.println("your input string is:\t"+str);
	...
	public static void main(String[] args) throws IOException, ExceptionofInput, ExceptionofUnproperEdge, ExceptionofUndirection {
